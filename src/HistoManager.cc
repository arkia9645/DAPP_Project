...
HistoManager::HistoManager(G4String fileOut)
  : fFactoryOn(false),
    ffileOut(fileOut)
{}
...
void HistoManager::Book()
{
  ...
  // Open an output file
  //
  G4bool fileOpen = analysisManager->OpenFile(ffileOut);
  if (! fileOpen) {
  ...
  if ( ! fFactoryOn ) {
    ...
    // id = 0
    analysisManager->CreateH1("EAbs","Edep in absorber (MeV)", 1400, 0., 700*keV);
    // id = 1
